  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        ...
        // 2️⃣ New Users (let’s say users in last 30 days)
        ...
        setStats([
          { label: "Total Sales", value: `$${totalSales?.toFixed(2) || 0}`, color: "#4caf50" },
        ...
        // 5️⃣ Recent Transactions (last 5 orders)
          ...
          <tbody>
            {recentOrders.map((order) => (
              <tr key={order.id}>
                ...
                <td style={{ padding: "12px" }}>{order.customer_name || order.user_email}</td>
                ...
                <td style={{ padding: "12px" }}>${Number(order.amount).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
